        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center gap-4">
                    <div v-if="user"
                        class="hidden sm:flex items-center gap-3 bg-slate-50 py-1.5 px-3 rounded-full border border-slate-200">
        ...
        <div v-show="isMenuOpen" class="sm:hidden bg-white border-b border-slate-200">
            <div class="pt-2 pb-3 space-y-1">
                <Link v-for="item in navigation" :key="item.name" :href="item.href"
                    ...
                    :class="item.active ? 'bg-blue-50 border-blue-500 text-blue-700' : 'border-transparent text-slate-500 hover:bg-slate-50 hover:border-slate-300 hover:text-slate-700'">
                    {{ item.name }}
                </Link>
            </div>
        </div>
